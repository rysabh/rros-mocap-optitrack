...
DEFAULT_PORT = (
    "/dev/serial/by-id/usb-Arduino_Srl_Arduino_Uno_55639303135351308212-if00"
)
...
class ArduinoBlueNode(Node):
    def __init__(self, *, port: str, baudrate: int, startup_off: bool) -> None:
        ...
        self.get_logger().info("Service: /arduino_blue/set_enabled (std_srvs/SetBool)")
    ...
    def send_command(self, command: str) -> None:
        ...
        self.get_logger().info(f"Sent '{command}' to blue Arduino")
        time.sleep(0.05)
...
def _interactive_loop(node: ArduinoBlueNode) -> None:
    while rclpy.ok():
        try:
            command = input(
                "Blue Arduino cmd (X stop, Y start, Z/L/Q timed): "
            ).strip().upper()
        except EOFError:
            return
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Blue Arduino controller (ROS2 service + optional CLI).")
    ...
    node = ArduinoBlueNode(port=args.port, baudrate=args.baudrate, startup_off=args.startup_off)
    ...
    try:
        ...
        node.get_logger().exception("arduino_blue crashed")
        return 1
    finally:
        node.shutdown()
        node.destroy_node()
